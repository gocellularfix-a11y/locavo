 ...
 *
 * V5.8.1: TODOS los arreglos de orden exportados quedan CONGELADOS en runtime
 * (`Object.freeze`), no solo `readonly` de tipo: un consumidor no puede mutar el
 * estado de orden compartido (push/splice/asignación de índice). Los valores son
 * primitivos, por lo que basta congelado superficial.
 */
import type {
  AccessibilityTrait,
  BestVisitTime,
...
/** Congela un arreglo de orden preservando el tipo de elemento de la unión. */
function freezeOrder<T extends string>(values: readonly T[]): readonly T[] {
  return Object.freeze(values.slice());
}

export const PERSONALITY_ORDER: readonly PlacePersonality[] = freezeOrder<PlacePersonality>([
...
export const VISIT_EXPERIENCE_ORDER: readonly VisitExperience[] = freezeOrder<VisitExperience>([
...
export const AUDIENCE_ORDER: readonly PlaceAudience[] = freezeOrder<PlaceAudience>([
  'FAMILIES', 'COUPLES', 'CHILDREN', 'STUDENTS', 'PROFESSIONALS', 'TOURISTS',
  'LOCALS', 'SENIORS', 'SOLO_VISITORS', 'GROUPS',
]);

export const BEST_TIME_ORDER: readonly BestVisitTime[] = freezeOrder<BestVisitTime>([
  'EARLY_MORNING', 'MORNING', 'BREAKFAST', 'LUNCH', 'AFTERNOON', 'SUNSET',
  'DINNER', 'EVENING', 'LATE_NIGHT', 'WEEKDAY', 'WEEKEND',
]);

export const NOISE_ORDER: readonly NoiseLevel[] = freezeOrder<NoiseLevel>(['QUIET', 'MODERATE', 'BUSY', 'LOUD']);

export const VISIT_DURATION_ORDER: readonly VisitDuration[] = freezeOrder<VisitDuration>([
  'UNDER_15_MIN', 'MIN_15_TO_30', 'MIN_30_TO_60', 'HOUR_1_TO_2', 'HALF_DAY', 'FULL_DAY',
]);

export const ACCESSIBILITY_ORDER: readonly AccessibilityTrait[] = freezeOrder<AccessibilityTrait>([
  'WHEELCHAIR_ACCESSIBLE', 'ACCESSIBLE_PARKING', 'GENERAL_PARKING', 'KID_FRIENDLY',
  'PET_FRIENDLY', 'OUTDOOR_ACCESS',
]);

export const EXPERIENCE_TAG_ORDER: readonly ExperienceTag[] = freezeOrder<ExperienceTag>([
...
export const SPECIALTY_ORDER: readonly PlaceSpecialty[] = freezeOrder<PlaceSpecialty>([
...
// V5.8.1: las señales de BestVisitTime (afinidad de categoría / apertura
// compatible) son de fuerza 1 → categoría+horas = MEDIUM, categoría sola = LOW,
// nunca HIGH.
const EVIDENCE_STRENGTH: Readonly<Record<PlaceIntelligenceEvidenceCode, 1 | 2 | 3>> = Object.freeze({
...
const EVIDENCE_SOURCE_ORDER: readonly PlaceIntelligenceEvidenceSource[] = Object.freeze([
  'CATEGORY', 'FEATURE', 'PRICE', 'HOURS', 'NAME_LEXICON',
] as const);
const EVIDENCE_CODE_ORDER: readonly PlaceIntelligenceEvidenceCode[] = Object.freeze([
  'CATEGORY_PRIMARY', 'CATEGORY_TIME_AFFINITY', 'FEATURE_WHEELCHAIR_ACCESSIBLE',
  ...
  'HOURS_TIME_WINDOW', 'HOURS_OPEN_WEEKDAY', 'HOURS_OPEN_WEEKEND', 'NAME_TOKEN',
] as const);
